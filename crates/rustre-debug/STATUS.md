...
> non un registro. Precedente riscrittura: 618. Questa: **624**, con due
...
|---|---|---|
| Windows x86_64 | suite locale, worktree isolato | **2095 / 1** |
| Linux x86_64 | WSL, `--test-threads=1` | **2078 / 1** |
...
| Cosa | Dove | Stato |
|---|---|---|
| **`step_out` non raggiunge il sito di ritorno** | iOS | Il messaggio si legge come «l'uscita chiamata ritorno», ma il test pretende che `step_out` **arrivi** a `pc == TEXT_BASE+0x0C` e il target corre fino all'uscita. Criterio: `pc == target && sp >= min_sp`, con `target` da `[fp+8]` e `min_sp = fp+16`; una delle due metà non si verifica mai. Punta dentro `apple_debugger.rs`, **non** a `run_to_return_step`, che su un'uscita vera fa bene a fermarsi. Del giro 9, che riscrive quel file adesso |
...
16. **Un `main` rosso batte un difetto nuovo** (623): la priorità è la build, non
