...
//! defined here outside this file's own `#[cfg(test)]` module — consistent
...
pub use super::{CvLineEntry, CvSymKind};
...
pub enum CvPdbSignature {
    ...
    Unknown {
        /// The unrecognized 4-byte magic value.
        magic: [u8; 4],
    },
}
...
impl CvParsedSymbolStream {
    ...
    pub fn function_count(&self) -> usize {
        self.symbols
            .iter()
            .filter(|s| s.kind.is_function() || matches!(s.kind, super::CvSymKind::Pub32))
            .count()
    }
...
impl CvSectionHeaders {
    ...
    pub fn parse(data: &[u8]) -> Self {
        ...
        while pos + 16 <= data.len() {
            ...
            // module_index lives at pos+2.
            let module_index = read_u16(data, pos + 2);
...
impl CvStreamParser {
    ...
    pub fn parse_debug_s(&mut self, data: &[u8]) -> Result<(), CodeViewError> {
        ...
        while pos + 8 <= data.len() {
            ...
            match Cv8SubsectionKind::from_u32(sub_type) {
                ...
                Cv8SubsectionKind::Lines => {
                    let blocks = super::parse_cv8_lines(payload)?;
                    for block in &blocks {
                        let file_name = raw_files
                ...
                Cv8SubsectionKind::FileChecksums => {
                    raw_files = parse_file_checksums_local(payload, &self.string_table);
                }
                _ => {}
            }
        ...
        // Second pass to resolve file names now that we have the string table.
        let mut line_infos: Vec<LineInfo> = Vec::with_capacity(line_infos_raw.len());
        for (file_index, mut li) in line_infos_raw {
            if li.file.is_empty()
                && let Some(f) = raw_files.get(file_index as usize)
            {
...
fn parse_file_checksums_local(data: &[u8], strtab: &CvStringTable) -> Vec<CvSourceFile> {
    let mut files = vec![];
    let mut pos = 0usize;
    while pos + 8 <= data.len() {
...
mod tests {
    use super::*;
    use crate::codeview::{build_test_gdata32, build_test_gproc32, build_test_lproc32, build_test_pub32};
