            ...
            if let Some(pid) = *self.pid.lock() {
                ...
                if let Ok(task_port) = resolve_task_port(pid.0 as libc::pid_t) {
                    ...
                    for tid in ids.unwrap_or_else(|_| vec![ThreadId(pid.0)]) {
                        ...
                        // Same distinction as the reporting loop, but there is
                        // no `still_armed` to answer into here. So an
                        // unverifiable set is handled by DOING the work instead
                        // of skipping it: writing zeros to a thread that had
                        // none costs a register write, while skipping one that
                        // had some leaves a trap armed in a process we are
                        // walking away from.
                        if matches!(
