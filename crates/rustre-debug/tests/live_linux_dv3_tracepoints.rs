...
fn build_fixture() -> Fixture {
    ...
    let src = dir.path().join("dv3trace_fx.c");
    let exe = dir.path().join("dv3trace_fx");
...
async fn zz_no_orphan_dv3_fixture_process_survives() {
    // `pgrep -x` matches the process NAME exactly, so the cargo/test command
    // lines that merely mention the fixture never count. A tracee killed a
    // moment ago can still be listed while it is reaped, so the check is
    // retried briefly: the assertion is about a SURVIVING process, not about
    // the exact instant of the last `kill`. That distinction is not cosmetic —
    // the loose `pgrep -f` spelling failed this suite once on a process that
    // was already exiting.
    let mut strays = String::new();
    for _ in 0..20 {
        let out = std::process::Command::new("pgrep")
            .args(["-x", "dv3trace_fx"])
            .output()
            .expect("pgrep must be available for the orphan check");
        strays = String::from_utf8_lossy(&out.stdout).trim().to_string();
        if strays.is_empty() {
            return;
        }
        std::thread::sleep(std::time::Duration::from_millis(100));
    }
    panic!("fixture processes survived the suite: {strays:?}");
}
