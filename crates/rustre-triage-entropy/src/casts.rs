...
pub fn f64_to_usize(x: f64) -> usize {
    ...
    // Extract the integer part of an f64 via IEEE 754 bit manipulation,
    // avoiding any `as`-cast that clippy::pedantic would reject.
    let bits = x.to_bits();
    let exp_raw = i32::try_from((bits >> 52) & 0x7FF).unwrap_or(0);
    let exp: i32 = exp_raw - 1023;
    if exp < 0 {
        return 0;
    }
    if exp >= 63 {
    ...
    let shift = 52i32 - exp;
    let int_val: u64 = if shift >= 0 {
        mantissa >> u32::try_from(shift).unwrap_or(0)
    } else {
        let lshift = u32::try_from(-shift).unwrap_or(0);
        if lshift >= 64 { u64::MAX } else { mantissa << lshift }
    };
    usize::try_from(int_val).unwrap_or(usize::MAX)
}
...
pub fn f64_to_u8(x: f64) -> u8 {
    // Reuse f64_to_usize (no `as`-cast, no unsafe) and clamp to u8 range.
    let v = f64_to_usize(x.min(255.0));
