...
#[async_trait::async_trait] impl ToolHandler for RsSymExtCrossRefIndexOpsTool { async fn call(&self, args: Value) -> Result<ToolResult, rustre_mcp_server::McpError> { let edges = args.get("edges").and_then(Value::as_array).ok_or_else(|| rustre_mcp_server::McpError::InvalidParams("missing edges".into()))?; let q = args.get("query").and_then(Value::as_u64).ok_or_else(|| rustre_mcp_server::McpError::InvalidParams("missing query".into()))?; let mut idx = rustre_symbols::CrossReferenceIndex::new(); for e in edges { if let Some(arr) = e.as_array() && arr.len() >= 2 { let f = arr[0].as_u64().unwrap_or(0); let t = arr[1].as_u64().unwrap_or(0); idx.add_xref(f, t); } } Ok(ToolResult::text(json!({"refs_to":idx.refs_to(q),"refs_from":idx.refs_from(q),"ref_count_to":idx.ref_count_to(q),"source":"rustre_symbols::CrossReferenceIndex"}).to_string())) } }
