...
#[async_trait] impl ToolHandler for DeobfSmcDecryptorDecryptTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let s: String = args.get("hex").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("missing 'hex'".into()))?.chars().filter(|c| !c.is_whitespace()).collect(); let data: Vec<u8> = (0..s.len()).step_by(2).filter_map(|i| u8::from_str_radix(s.get(i..i+2)?, 16).ok()).collect(); let key = args.get("key").and_then(Value::as_u64).unwrap_or(0); let algo = match args.get("algorithm").and_then(Value::as_str).unwrap_or("xor") { "add" => rustre_deobf_smc::SmcAlgorithm::Add, "sub" => rustre_deobf_smc::SmcAlgorithm::Sub, "rol" => rustre_deobf_smc::SmcAlgorithm::Rol, "ror" => rustre_deobf_smc::SmcAlgorithm::Ror, "xor_rolling" => rustre_deobf_smc::SmcAlgorithm::XorRolling, "add_rolling" => rustre_deobf_smc::SmcAlgorithm::AddRolling, _ => rustre_deobf_smc::SmcAlgorithm::Xor }; let region = rustre_deobf_smc::SmcRegion { start: 0, end: data.len() as u64, decryptor_addr: 0, key: rustre_deobf_smc::SmcKey::Constant(key), algorithm: algo }; let out = rustre_deobf_smc::SmcDecryptor::new().decrypt(&data, &region); let hex_out: String = out.iter().map(|b| format!("{b:02x}")).collect(); Ok(ToolResult::text(json!({"out_hex":hex_out,"len":out.len(),"source":"rustre_deobf_smc::SmcDecryptor::decrypt"}).to_string())) } }
