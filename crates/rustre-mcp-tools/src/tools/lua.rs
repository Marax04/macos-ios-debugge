...
impl LuaBcProtoStatsMockTool {
    #[must_use]
    pub fn definition() -> ToolDefinition {
        ToolDefinition { name: "lua_bc_proto_stats_mock".to_string(), description: "ProtoStats over mock LuaProto (rustre_loader_lua::ProtoStats::from_proto).".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}},"required":["version"]}), parameters: Value::Null }
    }
}
#[async_trait]
impl ToolHandler for LuaBcProtoStatsMockTool {
    async fn call(&self, args: Value) -> Result<ToolResult, McpError> {
        let vb = args.get("version").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("missing 'version'".into()))? as u8;
        let p = rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::from_byte(vb));
        let stats = rustre_loader_lua::ProtoStats::from_proto(&p);
...
impl LuaBcDisassembleMockTool {
    #[must_use]
    pub fn definition() -> ToolDefinition {
        ToolDefinition { name: "lua_bc_disassemble_mock".to_string(), description: "Disassemble mock LuaProto (rustre_loader_lua::disassemble_proto).".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}},"required":["version"]}), parameters: Value::Null }
    }
}
#[async_trait]
impl ToolHandler for LuaBcDisassembleMockTool {
    async fn call(&self, args: Value) -> Result<ToolResult, McpError> {
        let vb = args.get("version").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("missing 'version'".into()))? as u8;
        let p = rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::from_byte(vb));
        let lines = rustre_loader_lua::disassemble_proto(&p, vb);
        Ok(ToolResult::text(json!({"lines": lines, "count": p.instructions.len(), "source": "rustre_loader_lua::disassemble_proto"}).to_string()))
    }
}
...
impl LuaBcChunkFromMockTool {
    #[must_use]
    pub fn definition() -> ToolDefinition {
        ToolDefinition { name: "lua_bc_chunk_from_mock".to_string(), description: "LuaChunk from mock LuaProto (rustre_loader_lua::LuaChunk::from_proto).".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}},"required":["version"]}), parameters: Value::Null }
    }
}
#[async_trait]
impl ToolHandler for LuaBcChunkFromMockTool {
    async fn call(&self, args: Value) -> Result<ToolResult, McpError> {
        let vb = args.get("version").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("missing 'version'".into()))? as u8;
        let p = rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::from_byte(vb));
        let c = rustre_loader_lua::LuaChunk::from_proto(&p);
...
impl LuaBcModuleDisasmMockTool {
    #[must_use]
    pub fn definition() -> ToolDefinition {
        ToolDefinition { name: "lua_bc_module_disasm_mock".to_string(), description: "ModuleDisasm flat listing (rustre_loader_lua::ModuleDisasm).".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}},"required":["version"]}), parameters: Value::Null }
    }
}
#[async_trait]
impl ToolHandler for LuaBcModuleDisasmMockTool {
    async fn call(&self, args: Value) -> Result<ToolResult, McpError> {
        let vb = args.get("version").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("missing 'version'".into()))? as u8;
        let v = rustre_loader_lua::LuaVersion::from_byte(vb);
        let bc = rustre_loader_lua::LuaBytecode {
            header: rustre_loader_lua::LuaHeader { version: v, format: 0, endian: rustre_loader_lua::LuaEndian::Le, int_size: 4, ptr_size: 8, inst_size: 4, num_size: 8, is_integer_num: false, lua_integer_size: 8, lua_float_size: 8 },
            top_level: rustre_loader_lua::LuaProto::mock(v),
        };
        let module = rustre_loader_lua::LuaModule::from_bytecode(bc);
        let disasm = rustre_loader_lua::ModuleDisasm::from_module(&module);
...
impl LuaLoaderLuaProtoMockTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_mock".to_string(), description: "Build a mock LuaProto and report shape.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaProtoMockTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let counts = p.constant_type_counts(); Ok(ToolResult::text(json!({"display": p.to_string(), "total_instructions": p.total_instructions(), "num_constants": p.constants.len(), "num_upvalues": p.upvalues.len(), "num_locals": p.locals.len(), "constant_type_counts": counts, "source": "rustre_loader_lua::LuaProto::mock"}).to_string())) } }
...
impl LuaLoaderLuaAllStringsMockTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_all_strings_mock".to_string(), description: "Collect string constants of a mock LuaModule.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaAllStringsMockTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let root = rustre_loader_lua::LuaProto::mock(v); let module = rustre_loader_lua::LuaModule { header: rustre_loader_lua::LuaHeader { version: v, format: 0, endian: rustre_loader_lua::LuaEndian::Le, int_size: 4, ptr_size: 8, inst_size: 4, num_size: 8, is_integer_num: false, lua_integer_size: 8, lua_float_size: 8 }, root_proto: root }; let strs = rustre_loader_lua::LuaBytecodeLoader::all_strings(&module); Ok(ToolResult::text(json!({"strings": strs, "source": "rustre_loader_lua::LuaBytecodeLoader::all_strings"}).to_string())) } }

pub struct LuaLoaderLuaProtoStatsMockTool;
impl LuaLoaderLuaProtoStatsMockTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_stats_mock".to_string(), description: "Compute ProtoStats for a mock LuaProto.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaProtoStatsMockTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let s = rustre_loader_lua::ProtoStats::from_proto(&p); Ok(ToolResult::text(json!({"proto_count": s.proto_count, "instruction_count": s.instruction_count, "constant_count": s.constant_count, "string_count": s.string_count, "number_count": s.number_count, "integer_count": s.integer_count, "upvalue_count": s.upvalue_count, "local_count": s.local_count, "display": s.to_string(), "source": "rustre_loader_lua::ProtoStats::from_proto"}).to_string())) } }
...
impl LuaLoaderLuaDisassembleMockTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_disassemble_mock".to_string(), description: "Disassemble a mock LuaProto.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaDisassembleMockTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let lines = rustre_loader_lua::disassemble_proto(&p, vb); Ok(ToolResult::text(json!({"line_count": lines.len(), "lines": lines, "source": "rustre_loader_lua::disassemble_proto"}).to_string())) } }

pub struct LuaLoaderLuaChunkMockTool;
impl LuaLoaderLuaChunkMockTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_chunk_mock".to_string(), description: "Build a mock LuaChunk.".to_string(), input_schema: json!({"type":"object","required":["name"],"properties":{"name":{"type":"string"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaChunkMockTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let n = args.get("name").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("name".into()))?; let c = rustre_loader_lua::LuaChunk::mock(n); Ok(ToolResult::text(json!({"display": c.to_string(), "name": c.name, "first_line": c.first_line, "last_line": c.last_line, "num_params": c.num_params, "is_vararg": c.is_vararg, "max_stack": c.max_stack, "constants_count": c.constants_count, "functions_count": c.functions_count, "instructions_count": c.instructions_count, "source": "rustre_loader_lua::LuaChunk::mock"}).to_string())) } }
...
impl LuaLoaderLuaConstIsStringTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_const_is_string".to_string(), description: "Classify constants of a mock LuaProto.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaConstIsStringTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let items: Vec<_> = p.constants.iter().map(|c| json!({"display": c.to_string(), "is_string": c.is_string(), "as_str": c.as_str()})).collect(); Ok(ToolResult::text(json!({"count": items.len(), "constants": items, "source":"rustre_loader_lua::LuaConst::is_string"}).to_string())) } }

pub struct LuaLoaderLuaProtoSourceLineTool;
impl LuaLoaderLuaProtoSourceLineTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_source_line".to_string(), description: "Look up source line for a pc in a mock LuaProto.".to_string(), input_schema: json!({"type":"object","required":["version_byte","pc"],"properties":{"version_byte":{"type":"integer"},"pc":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaProtoSourceLineTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let pc = args.get("pc").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("pc".into()))? as usize; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let line = p.source_line(pc); Ok(ToolResult::text(json!({"pc": pc, "line": line, "line_info_len": p.line_info.len(), "source":"rustre_loader_lua::LuaProto::source_line"}).to_string())) } }

pub struct LuaLoaderLuaChunkFromProtoTool;
impl LuaLoaderLuaChunkFromProtoTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_chunk_from_proto".to_string(), description: "Build a LuaChunk from a mock LuaProto.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaChunkFromProtoTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let c = rustre_loader_lua::LuaChunk::from_proto(&p); Ok(ToolResult::text(json!({"display": c.to_string(), "name": c.name, "num_params": c.num_params, "is_vararg": c.is_vararg, "max_stack": c.max_stack, "constants_count": c.constants_count, "functions_count": c.functions_count, "instructions_count": c.instructions_count, "source":"rustre_loader_lua::LuaChunk::from_proto"}).to_string())) } }
...
impl LuaLoaderUpvalueDescFromUpvalueTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_upvalue_desc_from_upvalue".to_string(), description: "Convert a mock LuaProto's upvalues to UpvalueDesc.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderUpvalueDescFromUpvalueTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let descs: Vec<_> = p.upvalues.iter().map(|u| { let d = rustre_loader_lua::UpvalueDesc::from_upvalue(u); json!({"name": d.name, "in_stack": d.in_stack, "idx": d.idx, "display": d.to_string()}) }).collect(); Ok(ToolResult::text(json!({"count": descs.len(), "upvalues": descs, "source":"rustre_loader_lua::UpvalueDesc::from_upvalue"}).to_string())) } }

pub struct LuaLoaderLuaProtoAllStringsDirectTool;
impl LuaLoaderLuaProtoAllStringsDirectTool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_all_strings_direct".to_string(), description: "Collect LuaProto::all_strings from a mock proto directly.".to_string(), input_schema: json!({"type":"object","required":["version_byte"],"properties":{"version_byte":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait]
impl ToolHandler for LuaLoaderLuaProtoAllStringsDirectTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version_byte").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("version_byte".into()))? as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let strs: Vec<String> = p.all_strings().iter().map(|s| (*s).to_string()).collect(); Ok(ToolResult::text(json!({"count": strs.len(), "strings": strs, "source":"rustre_loader_lua::LuaProto::all_strings"}).to_string())) } }
...
impl LuaLoaderProtoTotalInstructionsWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_total_instructions_wx1".to_string(), description: "Build LuaProto::mock and return total_instructions via rustre_loader_lua::LuaProto::total_instructions.".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderProtoTotalInstructionsWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version").and_then(Value::as_u64).unwrap_or(0x54) as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); Ok(ToolResult::text(json!({"total_instructions":p.total_instructions(),"strings":p.all_strings(),"source_line_0":p.source_line(0),"source":"rustre_loader_lua::LuaProto::total_instructions"}).to_string())) } }

pub struct LuaLoaderProtoConstTypeCountsWx1Tool;
impl LuaLoaderProtoConstTypeCountsWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_const_type_counts_wx1".to_string(), description: "Report constant_type_counts on a mock LuaProto via rustre_loader_lua::LuaProto::constant_type_counts.".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderProtoConstTypeCountsWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version").and_then(Value::as_u64).unwrap_or(0x54) as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let counts = p.constant_type_counts(); let map: std::collections::BTreeMap<String,usize> = counts.iter().map(|(k,v)| ((*k).to_string(), *v)).collect(); Ok(ToolResult::text(json!({"counts":map,"num_constants":p.constants.len(),"source":"rustre_loader_lua::LuaProto::constant_type_counts"}).to_string())) } }

pub struct LuaLoaderChunkFromProtoFieldsWx1Tool;
impl LuaLoaderChunkFromProtoFieldsWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_chunk_from_proto_fields_wx1".to_string(), description: "Wrap LuaProto::mock into LuaChunk via rustre_loader_lua::LuaChunk::from_proto.".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderChunkFromProtoFieldsWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version").and_then(Value::as_u64).unwrap_or(0x54) as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let c = rustre_loader_lua::LuaChunk::from_proto(&p); Ok(ToolResult::text(json!({"name":c.name,"first_line":c.first_line,"last_line":c.last_line,"num_params":c.num_params,"is_vararg":c.is_vararg,"max_stack":c.max_stack,"constants_count":c.constants_count,"instructions_count":c.instructions_count,"functions_count":c.functions_count,"display":c.to_string(),"source":"rustre_loader_lua::LuaChunk::from_proto"}).to_string())) } }

pub struct LuaLoaderProtoStatsFromProtoWx1Tool;
impl LuaLoaderProtoStatsFromProtoWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_stats_from_proto_wx1".to_string(), description: "Compute ProtoStats over a mock LuaProto via rustre_loader_lua::ProtoStats::from_proto.".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderProtoStatsFromProtoWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version").and_then(Value::as_u64).unwrap_or(0x54) as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let s = rustre_loader_lua::ProtoStats::from_proto(&p); Ok(ToolResult::text(json!({"proto_count":s.proto_count,"instruction_count":s.instruction_count,"constant_count":s.constant_count,"string_count":s.string_count,"number_count":s.number_count,"integer_count":s.integer_count,"upvalue_count":s.upvalue_count,"local_count":s.local_count,"display":s.to_string(),"source":"rustre_loader_lua::ProtoStats::from_proto"}).to_string())) } }

pub struct LuaLoaderProtoWalkerCountWx1Tool;
impl LuaLoaderProtoWalkerCountWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_proto_walker_count_wx1".to_string(), description: "Walk protos DFS via rustre_loader_lua::ProtoWalker::new and count.".to_string(), input_schema: json!({"type":"object","properties":{"nested":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderProtoWalkerCountWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let n = args.get("nested").and_then(Value::as_u64).unwrap_or(2) as usize; let mut root = rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::Lua54); for _ in 0..n { root.protos.push(rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::Lua54)); } let count = rustre_loader_lua::ProtoWalker::new(&root).count(); Ok(ToolResult::text(json!({"count":count,"nested":n,"source":"rustre_loader_lua::ProtoWalker::new"}).to_string())) } }

pub struct LuaLoaderConstantIndexGetWx1Tool;
impl LuaLoaderConstantIndexGetWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_constant_index_get_wx1".to_string(), description: "Look up a constant by ConstantIndex via rustre_loader_lua::ConstantIndex::get.".to_string(), input_schema: json!({"type":"object","properties":{"index":{"type":"integer"}},"required":["index"]}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderConstantIndexGetWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let idx = args.get("index").and_then(Value::as_u64).ok_or_else(|| McpError::InvalidParams("missing 'index'".into()))? as u32; let p = rustre_loader_lua::LuaProto::mock(rustre_loader_lua::LuaVersion::Lua54); let ci = rustre_loader_lua::ConstantIndex(idx); let got = ci.get(&p); Ok(ToolResult::text(json!({"index":idx,"display":ci.to_string(),"found":got.is_some(),"value":got.map(|c| c.to_string()),"source":"rustre_loader_lua::ConstantIndex::get"}).to_string())) } }

pub struct LuaLoaderDisassembleProtoWx1Tool;
impl LuaLoaderDisassembleProtoWx1Tool { #[must_use] pub fn definition() -> ToolDefinition { ToolDefinition { name: "lua_loader_lua_disassemble_proto_wx1".to_string(), description: "Disassemble a mock LuaProto via rustre_loader_lua::disassemble_proto.".to_string(), input_schema: json!({"type":"object","properties":{"version":{"type":"integer"}}}), parameters: Value::Null } } }
#[async_trait] impl ToolHandler for LuaLoaderDisassembleProtoWx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let vb = args.get("version").and_then(Value::as_u64).unwrap_or(0x54) as u8; let v = rustre_loader_lua::LuaVersion::from_byte(vb); let p = rustre_loader_lua::LuaProto::mock(v); let lines = rustre_loader_lua::disassemble_proto(&p, vb); Ok(ToolResult::text(json!({"count":lines.len(),"lines":lines,"source":"rustre_loader_lua::disassemble_proto"}).to_string())) } }
