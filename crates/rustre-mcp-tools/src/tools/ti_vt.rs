...
#[async_trait]
impl ToolHandler for TiVtFileReportSpecStatsTool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let sha = args.get("sha256").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("sha256".into()))?; let r = rustre_ti_vt::mock_file_report(sha).map_err(|e| McpError::InvalidParams(format!("cannot build VT file report for {sha}: {e}")))?; Ok(ToolResult::text(json!({"is_malicious": r.is_malicious(), "malicious_count": r.malicious_count()}).to_string())) } }
