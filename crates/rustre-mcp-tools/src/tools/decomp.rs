...
#[async_trait] impl ToolHandler for DecompExpressionRecoveryRegisterDcx1Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let n = args.get("name").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("missing 'name'".into()))?.to_string(); let r = args.get("ret").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("missing 'ret'".into()))?.to_string(); let mut er = rustre_decompiler::ExpressionRecovery::new(); er.register_function(n.clone(), r); Ok(ToolResult::text(json!({"count":er.known_function_count(),"call_return_type":er.call_return_type(&n),"source":"rustre_decompiler::ExpressionRecovery"}).to_string())) } }
...
#[async_trait] impl ToolHandler for DecompTypeUnifierClassesZx2Tool { async fn call(&self, args: Value) -> Result<ToolResult, McpError> { let pairs = args.get("pairs").and_then(Value::as_array).ok_or_else(|| McpError::InvalidParams("missing 'pairs'".into()))?; let probe = args.get("probe").and_then(Value::as_str).ok_or_else(|| McpError::InvalidParams("missing 'probe'".into()))?.to_string(); let mut u = rustre_decompiler_type::TypeUnifier::new(); for p in pairs { if let Some(arr) = p.as_array() && arr.len() >= 2 && let (Some(a), Some(b)) = (arr[0].as_str(), arr[1].as_str()) { let c = rustre_decompiler_type::TypeConstraint::new(a, b, "eq"); u.add_constraint(&c); } } let can = u.canonical(&probe); let classes = u.equivalence_classes(); Ok(ToolResult::text(json!({"canonical": can, "class_count": classes.len(), "source":"rustre_decompiler_type::TypeUnifier::equivalence_classes"}).to_string())) } }
