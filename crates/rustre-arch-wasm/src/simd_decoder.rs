...
/// Sub-opcode dispatch chunk 0 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part0(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
    ...
    Ok(match sub {
        ...
        _ => return decode_simd_part1(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 1 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part1(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    Ok(match sub {
        ...
        _ => return decode_simd_part2(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 2 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part2(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    ...
    Ok(match sub {
        ...
        _ => return decode_simd_part3(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 3 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part3(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    ...
    Ok(match sub {
        ...
        _ => return decode_simd_part4(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 4 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part4(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    Ok(match sub {
        ...
        _ => return decode_simd_part5(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 5 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part5(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    Ok(match sub {
        ...
        _ => return decode_simd_part6(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 6 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part6(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
    Ok(match sub {
        ...
        _ => return decode_simd_part7(full_bytes, sub, pos),
    })
}

/// Sub-opcode dispatch chunk 7 of [`decode_simd`];
/// unmatched sub-opcodes fall through to the next chunk.
fn decode_simd_part7(
    full_bytes: &[u8],
    sub: u64,
    pos: &mut usize,
) -> Result<(&'static str, String, InstrFlags), CoreError> {
...
pub fn decode_simd(full_bytes: &[u8]) -> Result<(String, String, usize, InstrFlags), CoreError> {
    ...
    let (mnemonic, operands, flags): (&str, String, InstrFlags) =
        decode_simd_part0(full_bytes, sub, &mut pos)?;
...
impl V128Constant {
    ...
    /// # Panics
    ///
    /// Never panics: the source is a fixed 16-byte lane array, so each 4-byte
    /// chunk always converts.
    #[must_use]
    ...
    /// # Panics
    ///
    /// Never panics: the source is a fixed 16-byte lane array, so each 8-byte
    /// chunk always converts.
    #[must_use]
