...
// ─────────────────────────────────────────────────────────────────────────────
// Unified M68k register identifier
// ─────────────────────────────────────────────────────────────────────────────
//
// `m68k_calling_conventions` needs a single enum spanning both the data (D0-D7)
// and address (A0-A7) register files to describe argument/return/preserved
// register sets. The dedicated `m68k_registers` module splits these into
// `M68kDReg`/`M68kAReg`; this unified `M68kReg` is the flat identifier the
// calling-convention model indexes on.

/// A single M68k general-purpose register: data D0-D7 or address A0-A7 (A7=SP).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M68kReg {
...
impl M68kReg {
    /// Canonical assembler name, e.g. `"D3"` or `"A7"`.
    #[must_use]
