...
//!
//! Float-to-int conversions use `f64::to_int_unchecked` / `f32::to_int_unchecked`
//! inside an `unsafe` block, but only after clamping the input to a finite
//! in-range value, which makes the operation defined per the stdlib safety
//! contract (finite, in-range value).
...
pub fn f64_to_usize(x: f64) -> usize {
    ...
    // Compare against 2^64 directly rather than deriving the bound from
    // `u64_to_f64(u64::MAX)`: that helper deliberately saturates at 2^53 (its
    // own documented contract, to stay inside f64's exact-integer range), so
    // using it as the clamp silently made *this* function saturate at 2^53 too
    // — `f64_to_usize(1e300)` returned 9_007_199_254_740_992 instead of the
    // `usize::MAX` promised above.
    //
    // 2^64 is the smallest f64 that does not fit in `u64`, and it is exactly
    // representable, so `>=` is the precise boundary.
    const TWO_POW_64: f64 = 18_446_744_073_709_551_616.0;
    if x >= TWO_POW_64 {
        return usize::MAX;
    }
    // SAFETY: `x` is finite, strictly positive and strictly below 2^64, so
    // truncation toward zero yields a value representable in `u64`.
    let v: u64 = unsafe { x.to_int_unchecked::<u64>() };
...
pub fn f64_to_u8(x: f64) -> u8 {
    ...
    let clamped = x.min(255.0);
    // SAFETY: `clamped` ∈ [0.0, 255.0] and finite — defined for u8 target.
    unsafe { clamped.to_int_unchecked::<u8>() }
}
