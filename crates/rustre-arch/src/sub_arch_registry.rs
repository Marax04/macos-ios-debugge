//! `sub_arch_registry` — central dispatcher that wires every concrete
//! `rustre-arch-*` sub-crate into the hub's [`crate::global_registry`].
//!
...
//! into the process-wide [`crate::global_registry`], overwriting any
//! `PlaceholderArch` entries that were previously inserted by
//! [`crate::register_all_builtins`].
...
/// stubs from [`crate::register_all_builtins`] — are overwritten.
pub fn register_all() {
