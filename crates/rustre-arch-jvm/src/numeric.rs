//! Checked, panic-free numeric conversions used across the JVM decoders.
//!
//! Every function here replaces a bare `as` cast at a site that is fed
//! attacker-controlled bytes (a class-file constant pool, an attribute
//! blob, a raw bytecode stream). A `.unwrap()` on such a path would be a denial of
...
/// out-of-range offset in a malformed class file cannot alias a valid
/// one.
#[inline]
#[must_use]
...
// narrowing IS the specified behaviour (a JVM local-variable index is 16 bits wide by
