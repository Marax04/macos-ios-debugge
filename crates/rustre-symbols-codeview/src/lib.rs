// [DISABLED — ABSORBED INTO rustre-debug 2026-07-12]
...
use rustre_symbols::{SourceLocation, SymKind, Symbol, SymbolProvider, TypeInfo};
...
impl CvSignature {
    ...
    pub fn from_bytes(b: &[u8]) -> Option<Self> {
        ...
        match &b[0..4] {
            ...
            // "RSDS" is the PDB 7.0 tag found in PE debug-directory records;
            // "Micr" is the start of the MSF 7.0 file magic
            // ("Microsoft C/C++ MSF 7.00\r\n"). Both indicate PDB 7.0.
            b"RSDS" | b"Micr" => Some(Self::Pdb70),
...
fn parse_one_symbol(kind: CvSymKind, data: &[u8], out: &mut Vec<CvSymbol>) {
    match kind {
        // S_GPROC32 / S_LPROC32 (Microsoft PROCSYM32 layout):
        // [parent:u32][end:u32][next:u32][proc_len:u32]
        // [debug_start:u32][debug_end:u32][type_index:u32][offset:u32]
        // [seg:u16][flags:u8][name\0]
        CvSymKind::GProc32 | CvSymKind::LProc32 => {
            if data.len() < 35 {
                return;
            }
            let type_index = read_u32(data, 24);
...
fn parse_one_type(kind: CvTypeKind, data: &[u8], index: u32) -> CvTypeRecord {
    match kind {
        ...
        CvTypeKind::Structure | CvTypeKind::Class | CvTypeKind::Union => {
            ...
            // The size field is a variable-length numeric leaf: for aggregates
            // >= 0x8000 bytes the u16 at 16 is an LF_USHORT/LF_ULONG tag and the
            // real value (and hence the name) follow at a data-dependent offset.
            let (size_raw, name_off) = parse_numeric_leaf(data, 16).unwrap_or((0, 18));
            let size = u32::try_from(size_raw).unwrap_or(u32::MAX);
            let name = read_null_str(data, name_off);
            CvTypeRecord {
                kind,
                index,
                name,
        ...
        CvTypeKind::Pointer => {
            ...
            CvTypeRecord {
                ...
                size: if data.len() >= 8 {
                    ...
                    // LF_POINTER attributes: ptrtype in bits 0..5,
                    // ptrmode in bits 5..8, explicit size in bits 13..19.
                    let explicit_size = (attr >> 13) & 0x3F;
                    if explicit_size != 0 {
                        u32::min(explicit_size, 255)
                    } else if (attr & 0x1F) == 0x0C {
                        // CV_PTR_64 (near 64-bit pointer)
                        8
                    } else {
                        4
                    }
...
impl CvTypeTable {
    ...
    pub fn to_type_info(&self, index: u32) -> TypeInfo {
        self.to_type_info_depth(index, 0)
    }

    /// Depth-limited worker for [`Self::to_type_info`].
    ///
    /// A self-referential TPI record (e.g. an `LF_POINTER` whose referent is
    /// itself) would otherwise recurse until the stack is exhausted, which
    /// aborts the process uncatchably.
    fn to_type_info_depth(&self, index: u32, depth: u32) -> TypeInfo {
        ...
        match self.lookup(index) {
            ...
            Some(rec) => match rec.kind {
                CvTypeKind::Pointer => TypeInfo::Pointer {
                    target: Box::new(self.to_type_info_depth(rec.underlying_type, depth + 1)),
                    size: u8::try_from(rec.size).unwrap_or(u8::MAX),
                },
                CvTypeKind::Structure | CvTypeKind::Class | CvTypeKind::Union => {
                    // The TPI summary record only carries the member count and
                    // aggregate size; accurate per-member offsets and types
                    // require the LF_FIELDLIST record (see `cv_type_records`).
                    // Earlier versions fabricated `size / count`-strided
                    // placeholder offsets here, which were indistinguishable
                    // from real layout data for consumers. Emit an empty field
                    // list instead: absent data is honest, invented data is not.
                    TypeInfo::Struct {
                        name: rec.name.clone(),
                        fields: vec![],
                    }
                ...
                CvTypeKind::Enum => TypeInfo::Enum {
                    name: rec.name.clone(),
                    variants: vec![],
                    base_type: Box::new(self.to_type_info_depth(rec.underlying_type, depth + 1)),
                },
                CvTypeKind::Array => TypeInfo::Array {
                    element: Box::new(self.to_type_info_depth(rec.underlying_type, depth + 1)),
...
pub fn primitive_type(index: u32) -> TypeInfo {
    // CV_typ encoding: bits [11:8] are the pointer mode, bits [7:0] the base type.
    // Modes: 0 = direct, 1 = near ptr, 2 = far, 3 = huge,
    //        4 = 32-bit near ptr, 5 = 32-bit far ptr, 6 = 64-bit near ptr.
    let mode = (index >> 8) & 0xF;
    ...
    if mode != 0 {
        ...
        return TypeInfo::Pointer {
            ...
            size: if mode == 6 { 8 } else { 4 },
        };
    }
    match base_type {
        // 0x0000 is T_NOTYPE (absent), not void; T_VOID is 0x0003.
        0x03 => TypeInfo::Void,
        // T_RCHAR (0x70) is an 8-bit char, not a 16-bit int.
        0x10 | 0x68 | 0x70 => TypeInfo::Int {
            width: 8,
            signed: true,
        },
        ...
        0x11 | 0x72 => TypeInfo::Int {
            width: 16,
            signed: true,
        },
        // T_WCHAR (0x71) is 16-bit unsigned.
...
pub fn parse_cv8_lines(data: &[u8]) -> Result<Vec<Cv8LineBlock>, CodeViewError> {
    ...
    while pos + 12 <= data.len() {
        ...
        for i in 0..num_lines {
            ...
            // Flags word packs lineStart:24, deltaLineEnd:7, isStatement:1.
            let line_start = line_num_raw & 0x00FF_FFFF;
            let is_statement = (line_num_raw >> 31) != 0;
...
pub struct CvDebugSection {
    ...
    /// Map from the byte offset of each entry inside the `FileChecksums`
    /// (`0xF4`) subsection to its position in [`Self::source_files`].
    ///
    /// CV8 `DEBUG_S_LINES` blocks reference files by this byte offset, NOT by
    /// ordinal index.
    pub source_file_offsets: HashMap<u32, usize>,
}
...
impl CvDebugSection {
    ...
    pub fn parse(data: &[u8]) -> Result<Self, CodeViewError> {
        ...
        let mut pos = 4usize;
        // The spec does not guarantee subsection order: the string table
        // (0xF3) may appear after the file checksums (0xF4). Defer checksum
        // decoding to a second pass so file names always resolve.
        ...
        // Second pass: decode file checksums now that the string table is known.
        for (start, end) in checksum_ranges {
            let (files, offsets) =
                parse_file_checksums_with_offsets(&data[start..end], &result.string_table);
            let base = result.source_files.len();
            for (off, idx) in offsets {
                result.source_file_offsets.insert(off, base + idx);
            }
...
/// Parse a `FileChecksums` (`0xF4`) subsection, additionally returning a map
/// from each entry's byte offset within the subsection to its index in the
/// returned vector. CV8 line blocks reference files by that byte offset.
fn parse_file_checksums_with_offsets(
    data: &[u8],
    strtab: &CvStringTable,
) -> (Vec<CvSourceFile>, HashMap<u32, usize>) {
    let mut files = vec![];
    ...
    while pos + 8 <= data.len() {
        ...
        offsets.insert(u32::try_from(entry_offset).unwrap_or(u32::MAX), files.len());
...
/// Resolve a `CodeView` (segment, offset) pair to a virtual address.
///
/// `segment` is 1-based and indexes `section_rvas` (the RVA of each PE
/// section, in section-table order). Returns `None` for segment 0
/// (absolute / internal-linkage symbols), for an out-of-range segment, and
/// on arithmetic overflow.
#[must_use]
pub fn resolve_cv_va(image_base: u64, section_rvas: &[u64], segment: u16, offset: u32) -> Option<u64> {
    if segment == 0 {
        return None;
    }
    let rva = *section_rvas.get(usize::from(segment) - 1)?;
    image_base
        .checked_add(rva)
        .and_then(|b| b.checked_add(u64::from(offset)))
}
...
impl CodeViewProvider {
    ...
    pub fn from_bytes(data: &[u8], image_base: u64) -> Result<Self, CodeViewError> {
        ...
        let symbols = raw_syms
            ...
            .map(|cv| {
                ...
                Symbol::new(
                    cv.name.clone(),
                    image_base.saturating_add(u64::from(cv.offset)),
                    kind,
                )
            })
            .collect();
    ...
    /// Build a provider from a `CodeView` symbol stream, resolving each
    /// symbol's virtual address through the PE section table.
    ///
    /// `section_rvas` holds the RVA of each PE section in section-table
    /// order; a `CodeView` symbol's 1-based `segment` indexes it. Symbols
    /// with segment 0 (absolute / internal linkage) or an out-of-range
    /// segment are skipped rather than being mapped to a bogus address.
    ///
    /// Prefer this over [`Self::from_bytes`], which ignores the segment and
    /// therefore places every symbol outside the first section incorrectly.
    ///
    /// # Errors
    ///
    /// Returns [`CodeViewError`] if `data` cannot be parsed.
    pub fn from_bytes_with_sections(
        data: &[u8],
        image_base: u64,
        section_rvas: &[u64],
        ...
        let symbols = raw_syms
            .iter()
            .filter_map(|cv| {
                let addr = resolve_cv_va(image_base, section_rvas, cv.segment, cv.offset)?;
                let kind = if cv.kind.is_function() {
                    SymKind::Function
                } else {
                    SymKind::Data
                };
                Some(Symbol::new(cv.name.clone(), addr, kind))
            })
            .collect();
        Ok(Self {
            symbols,
    ...
    pub fn from_debug_section(data: &[u8], image_base: u64) -> Result<Self, CodeViewError> {
        Self::debug_section_inner(data, image_base, None)
    }

    /// Build a provider from a complete CV8 `.debug$S` section, resolving
    /// addresses through the PE section table.
    ///
    /// See [`Self::from_bytes_with_sections`] for the meaning of
    /// `section_rvas` and the skipping rules.
    ///
    /// # Errors
    ///
    /// Returns [`CodeViewError`] if parsing fails.
    pub fn from_debug_section_with_sections(
        data: &[u8],
        image_base: u64,
        section_rvas: &[u64],
    ) -> Result<Self, CodeViewError> {
        Self::debug_section_inner(data, image_base, Some(section_rvas))
    }

    fn debug_section_inner(
        data: &[u8],
        image_base: u64,
        section_rvas: Option<&[u64]>,
    ) -> Result<Self, CodeViewError> {
        let section = CvDebugSection::parse(data)?;
        let resolve = |segment: u16, offset: u32| -> Option<u64> {
            match section_rvas {
                Some(rvas) => resolve_cv_va(image_base, rvas, segment, offset),
                None => Some(image_base.saturating_add(u64::from(offset))),
            }
        };
        let symbols = section
            .symbols
            .iter()
            .filter_map(|cv| {
                let addr = resolve(cv.segment, cv.offset)?;
                let kind = if cv.kind.is_function() {
                    SymKind::Function
                } else {
                    SymKind::Data
        ...
        for block in &section.line_blocks {
            // CV8 stores the file reference as a byte offset into the
            // FileChecksums subsection, not an ordinal index.
            let file_name = section
                .source_file_offsets
                .get(&block.file_index)
                .and_then(|&i| section.source_files.get(i))
                .map(|f| f.name.clone())
                .unwrap_or_default();
            for entry in &block.lines {
                let Some(addr) = resolve(block.segment, block.code_offset)
                    .and_then(|base| base.checked_add(u64::from(entry.offset)))
                else {
                    continue;
                };
                source_map.push((
...
pub fn build_test_gproc32(name: &str, offset: u32, seg: u16, type_index: u32) -> Vec<u8> {
    // S_GPROC32 layout (record_data part, Microsoft PROCSYM32):
    // [parent:u32][end:u32][next:u32][proc_len:u32]
    // [debug_start:u32][debug_end:u32][type_index:u32][offset:u32]
    // [seg:u16][flags:u8][name\0]
    let mut payload: Vec<u8> = Vec::new();
    payload.extend_from_slice(&0u32.to_le_bytes()); // parent @ 0
    payload.extend_from_slice(&0u32.to_le_bytes()); // end @ 4
    payload.extend_from_slice(&0u32.to_le_bytes()); // next @ 8
    payload.extend_from_slice(&0u32.to_le_bytes()); // proc_len @ 12
    payload.extend_from_slice(&0u32.to_le_bytes()); // debug_start @ 16
    payload.extend_from_slice(&0u32.to_le_bytes()); // debug_end @ 20
    payload.extend_from_slice(&type_index.to_le_bytes()); // type_index @ 24
...
pub fn build_test_pub32(name: &str, offset: u32) -> Vec<u8> {
    ...
    payload.extend_from_slice(name.as_bytes());
    payload.push(0);
    let kind: u16 = 0x110E; // S_PUB32 (0x1009 is the legacy S_PUB32_ST)
    let len = u16::try_from(2 + payload.len()).unwrap_or(u16::MAX);
    let mut record = Vec::new();
    record.extend_from_slice(&len.to_le_bytes());
    record.extend_from_slice(&kind.to_le_bytes());
...
impl CodeViewMagic {
    /// The raw `u32` tag value for this magic.
...
fn rva_to_foff(
    ...
    for i in 0..num_sections {
        ...
        // IMAGE_SECTION_HEADER: Name[8] @0, VirtualSize @8, VirtualAddress @12,
        // SizeOfRawData @16, PointerToRawData @20.  This read only +16
        // (SizeOfRawData), so an RVA in a section's uninitialised tail — where
        // VirtualSize exceeds SizeOfRawData — was never resolved.  Neither
        // field alone is enough: VirtualSize is 0 in object files.  Take the
        // larger, which is what the mapped section actually spans.
        let virt_size =
            (read_u32(pe_data, sec + 8) as usize).max(read_u32(pe_data, sec + 16) as usize);
...
impl CvFrameproc {
    /// Parse from the record payload (26 bytes minimum).
    ///
    /// FRAMEPROCSYM layout: cbFrame\@0, cbPad\@4, offPad\@8, cbSaveRegs\@12,
    /// offExHdlr:u32\@16, sectExHdlr:u16\@20, flags:u32\@22 — a 26-byte payload.
    #[must_use]
    pub fn parse(data: &[u8]) -> Option<Self> {
        if data.len() < 26 {
            return None;
        ...
        Some(Self {
            ...
            eh_offset: read_u32(data, 16),
            eh_section: read_u16(data, 20),
            flags: read_u32(data, 22),
        })
    }

    /// Returns `true` if the function uses `__alloca` (fHasAlloca, bit 0).
    #[must_use]
    pub const fn has_alloca(&self) -> bool {
        self.flags & 0x0000_0001 != 0
    }

    /// Returns `true` if the function uses `setjmp` (fHasSetJmp).
    #[must_use]
    pub const fn has_setjmp(&self) -> bool {
        self.flags & 0x0000_0002 != 0
    }

    /// Returns `true` if the function uses `longjmp` (fHasLongJmp).
    #[must_use]
    pub const fn has_longjmp(&self) -> bool {
        self.flags & 0x0000_0004 != 0
    }

    /// Returns `true` if the function contains inline assembly (fHasInlAsm).
    #[must_use]
    pub const fn has_inline_asm(&self) -> bool {
        self.flags & 0x0000_0008 != 0
    }

    /// Returns `true` if the function has C++ exception handling (fHasEH).
    #[must_use]
    pub const fn has_eh(&self) -> bool {
        self.flags & 0x0000_0010 != 0
    }

    /// Returns `true` if the function was marked `inline` (fInlSpec).
    #[must_use]
    pub const fn is_inline_specified(&self) -> bool {
        self.flags & 0x0000_0020 != 0
    }

    /// Returns `true` if the function has structured exception handling (fHasSEH).
    #[must_use]
    pub const fn has_seh(&self) -> bool {
        self.flags & 0x0000_0040 != 0
    }

    /// Returns `true` if the function is `__declspec(naked)` (fNaked).
    #[must_use]
    pub const fn is_naked(&self) -> bool {
        self.flags & 0x0000_0080 != 0
    }

    /// Returns `true` if the function has stack security checks (fSecurityChecks).
    #[must_use]
    pub const fn has_security_checks(&self) -> bool {
        self.flags & 0x0000_0100 != 0
    }

    /// Returns `true` if the function has asynchronous EH (fAsyncEH).
    #[must_use]
    pub const fn has_async_eh(&self) -> bool {
        self.flags & 0x0000_0200 != 0
    }

    /// Encoded local base pointer register (bits 14-15).
    #[must_use]
    pub const fn encoded_local_base_ptr(&self) -> u32 {
        (self.flags >> 14) & 0x3
    }

    /// Encoded parameter base pointer register (bits 16-17).
    #[must_use]
    pub const fn encoded_param_base_ptr(&self) -> u32 {
        (self.flags >> 16) & 0x3
    }
}
...
fn parse_numeric_leaf(data: &[u8], off: usize) -> Option<(u64, usize)> {
    ...
    match tag {
        ...
        0x8001 => {
            ...
            Some((crate::casts::i16_sext_u64(v), off + 4))
        }
        0x8002 => {
            // LF_USHORT
            if off + 4 > data.len() {
        ...
        0x8003 => {
            ...
            Some((crate::casts::i32_sext_u64(v), off + 6))
        }
        0x8004 => {
            // LF_ULONG
            if off + 6 > data.len() {
...
pub struct PdbSuperBlock {
    ...
    /// Reserved/unknown field at offset 48 in the super-block.
    pub unknown: u32,
...
mod tests {
    ...
    fn test_cv_sym_kind_roundtrip() {
        assert_eq!(CvSymKind::from_u16(0x110E), CvSymKind::Pub32);
        // 0x1009 is S_PUB32_ST (length-prefixed name), not the modern S_PUB32.
        assert_eq!(CvSymKind::from_u16(0x1009), CvSymKind::Unknown);
    ...
    #[test]
    fn test_type_table_struct_fields_empty_without_fieldlist() {
        // The TPI summary record carries only count and size; without the
        // LF_FIELDLIST no per-member layout may be invented.
        let data = build_struct_type_record("MyStruct", 3, 24);
        let t = CvTypeTable::from_bytes(&data).unwrap();
    ...
    fn test_type_table_primitive_void() {
        let t = CvTypeTable::new();
        // T_VOID is 0x0003. 0x0000 is T_NOTYPE (no type recorded), not void.
        assert!(matches!(t.to_type_info(0x03), TypeInfo::Void));
        assert!(matches!(t.to_type_info(0x00), TypeInfo::Unknown));
    }

    #[test]
    fn test_sig_rsds_pdb70() {
        // RSDS is the PDB 7.0 tag in PE debug-directory records.
        assert_eq!(
            CvSignature::from_bytes(b"RSDSxxxx"),
            Some(CvSignature::Pdb70)
        );
    }

    #[test]
    fn test_primitive_pointer_mode_sizes() {
        // T_32PVOID (mode 4 = 32-bit near pointer) -> 4 bytes.
        match primitive_type(0x0403) {
            TypeInfo::Pointer { size, .. } => assert_eq!(size, 4),
            other => panic!("expected pointer, got {other}"),
        }
        // T_64PVOID (mode 6 = 64-bit near pointer) -> 8 bytes.
        match primitive_type(0x0603) {
            TypeInfo::Pointer { size, .. } => assert_eq!(size, 8),
            other => panic!("expected pointer, got {other}"),
        }
    }

    fn build_pointer_type_record(target: u32, attr: u32) -> Vec<u8> {
        let mut body = vec![];
    ...
    #[test]
    fn test_lf_pointer_near64_size() {
        // ptrtype = 0x0C (CV_PTR_64), no explicit size field -> 8 bytes.
        let recs = parse_cv_type_records(&build_pointer_type_record(0x74, 0x0C)).unwrap();
        assert_eq!(recs[0].size, 8);
        // ptrtype = 0x0A (CV_PTR_NEAR32) -> 4 bytes.
        let recs = parse_cv_type_records(&build_pointer_type_record(0x74, 0x0A)).unwrap();
        assert_eq!(recs[0].size, 4);
        // Explicit size field (bits 13..19) wins: size = 8.
        let recs = parse_cv_type_records(&build_pointer_type_record(0x74, 8 << 13)).unwrap();
        assert_eq!(recs[0].size, 8);
    }

    #[test]
    fn test_gproc32_type_index_at_offset_24() {
        // Raw PROCSYM32 layout: type_index lives at payload offset 24, not 0.
        let mut payload = vec![0u8; 35];
        payload[0..4].copy_from_slice(&0xAAAA_AAAAu32.to_le_bytes()); // parent (decoy)
        payload[24..28].copy_from_slice(&0x77u32.to_le_bytes()); // type_index
        payload[28..32].copy_from_slice(&0x2000u32.to_le_bytes()); // offset
        payload[32..34].copy_from_slice(&1u16.to_le_bytes()); // seg
        payload.extend_from_slice(b"f\0");
        let mut rec = vec![];
        let len = u16::try_from(2 + payload.len()).unwrap();
        rec.extend_from_slice(&len.to_le_bytes());
        rec.extend_from_slice(&0x1110u16.to_le_bytes()); // S_GPROC32
        rec.extend_from_slice(&payload);
        let syms = parse_cv_symbols(&rec).unwrap();
        assert_eq!(syms.len(), 1);
        assert_eq!(syms[0].type_index, 0x77);
        assert_eq!(syms[0].offset, 0x2000);
        assert_eq!(syms[0].name, "f");
    }

    #[test]
    fn test_pub32_modern_tag_0x110e() {
        let bytes = build_test_pub32("PubSym", 0x40);
        // The builder must emit the modern S_PUB32 tag.
        assert_eq!(u16::from_le_bytes([bytes[2], bytes[3]]), 0x110E);
        let syms = parse_cv_symbols(&bytes).unwrap();
        assert_eq!(syms[0].kind, CvSymKind::Pub32);
        assert_eq!(syms[0].name, "PubSym");
        // The legacy 0x1009 (S_PUB32_ST) must no longer map to Pub32.
        assert_eq!(CvSymKind::from_u16(0x1009), CvSymKind::Unknown);
    }

    /// Build a `.debug$S` section with: F2 lines (file byte-offset ref),
    /// F4 checksums (two entries), F3 string table LAST (order stress).
    fn build_debug_s_two_files() -> Vec<u8> {
        let mut sec = vec![];
        sec.extend_from_slice(&4u32.to_le_bytes()); // CV8 version

        // F2 DEBUG_S_LINES: header + one block referencing the SECOND file,
        // whose F4 entry starts at byte offset 8 (each entry is 8 bytes:
        // 6-byte header + 1-byte checksum + pad).
        let mut lines = vec![];
        lines.extend_from_slice(&0x1000u32.to_le_bytes()); // code_offset
        lines.extend_from_slice(&1u16.to_le_bytes()); // segment
        lines.extend_from_slice(&0u16.to_le_bytes()); // flags
        lines.extend_from_slice(&0x20u32.to_le_bytes()); // code_len
        lines.extend_from_slice(&8u32.to_le_bytes()); // file ref = BYTE OFFSET 8
        lines.extend_from_slice(&1u32.to_le_bytes()); // num_lines
        lines.extend_from_slice(&20u32.to_le_bytes()); // block_size
        lines.extend_from_slice(&0u32.to_le_bytes()); // line offset
        lines.extend_from_slice(&42u32.to_le_bytes()); // line 42
        sec.extend_from_slice(&0xF2u32.to_le_bytes());
        sec.extend_from_slice(&u32::try_from(lines.len()).unwrap().to_le_bytes());
        sec.extend_from_slice(&lines);

        // F4 FileChecksums: two entries, 8 bytes each.
        let mut cks = vec![];
        for name_off in [1u32, 7u32] {
            cks.extend_from_slice(&name_off.to_le_bytes());
            cks.push(1); // checksum len
            cks.push(0); // kind
            cks.push(0xAB); // checksum byte
            cks.push(0); // pad to 8
        }
        sec.extend_from_slice(&0xF4u32.to_le_bytes());
        sec.extend_from_slice(&u32::try_from(cks.len()).unwrap().to_le_bytes());
        sec.extend_from_slice(&cks);

        // F3 StringTable placed AFTER F4: "\0a.cpp\0b.cpp\0" (offset 1 = a.cpp, 7 = b.cpp).
        let strtab = b"\0a.cpp\0b.cpp\0";
        sec.extend_from_slice(&0xF3u32.to_le_bytes());
        sec.extend_from_slice(&u32::try_from(strtab.len()).unwrap().to_le_bytes());
        sec.extend_from_slice(strtab);
        while sec.len() % 4 != 0 {
            sec.push(0);
        }
        sec
    ...
    #[test]
    fn test_debug_section_file_ref_is_byte_offset_and_order_independent() {
        let sec = build_debug_s_two_files();
        let parsed = CvDebugSection::parse(&sec).unwrap();
        // String table came after checksums: names must still resolve.
        assert_eq!(parsed.source_files.len(), 2);
        assert_eq!(parsed.source_files[0].name, "a.cpp");
        assert_eq!(parsed.source_files[1].name, "b.cpp");
        // Byte offset 8 must map to the SECOND file, not index 8.
        assert_eq!(parsed.source_file_offsets.get(&8), Some(&1));

        let provider = CodeViewProvider::from_debug_section(&sec, 0x40_0000).unwrap();
        let loc = provider
            .source_map
            .iter()
            .find(|(_, l)| l.line == 42)
            .map(|(_, l)| l.clone())
            .expect("line 42 present");
        assert_eq!(loc.file, "b.cpp");
    }
