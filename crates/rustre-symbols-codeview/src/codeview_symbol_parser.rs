...
pub enum SymParseError {
    ...
    RecordTooShort { kind: u16, len: usize },
...
pub struct Compile3Symbol {
    ...
    pub be_build: u16,
    /// Front-end version QFE.
    pub fe_qfe: u16,
    /// Back-end version QFE.
    pub be_qfe: u16,
...
impl Compile3Symbol {
    ...
    #[must_use]
    pub fn parse(data: &[u8]) -> Option<Self> {
        // COMPILESYM3: flags:u32@0, machine:u16@4, then four front-end version
        // u16s (major, minor, build, QFE) and four back-end ones, so the
        // version string starts at offset 22 — not 18.
        if data.len() < 22 {
            return None;
        }
        Some(Self {
            ...
            fe_qfe: read_u16(data, 12),
            be_major: read_u16(data, 14),
            be_minor: read_u16(data, 16),
            be_build: read_u16(data, 18),
            be_qfe: read_u16(data, 20),
            version_string: cstr(data, 22),
        })
    }
...
impl CodeViewSymbolParser {
    ...
    fn parse_one(kind: u16, payload: &[u8]) -> ParsedSymbol {
        match kind {
            ...
            // S_PUB32 = 0x110E (0x1009 is the legacy S_PUB32_ST)
            0x110E => PublicSymbol::parse(payload).map_or_else(
                || ParsedSymbol::Raw { kind, data: payload.to_vec() },
                ParsedSymbol::Public,
            ),
            // S_LOCAL = 0x113E
